...
const ProgramDetail = ({ onNavigateToPlan }) => {
	...
	if (loading) {
		return (
			<div className="bg-white mt-2 border border-slate-200 rounded-xl shadow-sm p-6 text-sm text-slate-500">
	...
	if (error) {
		return (
			<div className="bg-white mt-2 border border-slate-200 rounded-xl shadow-sm p-6 text-sm text-rose-600">
				{error}
			</div>
		);
	}
			...
			<div className="grid gap-4 md:grid-cols-2">
				<article className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 space-y-3">
					...
					<ul className="text-sm text-slate-700 space-y-2">
						...
						<li>
							Graduation rate: {formatPercent(university.graduation_rate)}
						</li>
						<li>
							Average annual cost:{" "}
						...
						<li>
							...
							{formatCurrency(university.median_earnings)}
						</li>
					</ul>
					{websiteUrl && (
						<a
				...
				<article className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 space-y-3">
					...
					<ul className="text-sm text-slate-700 space-y-2">
						...
						<li>
							Acceptance rate: {formatPercent(university.acceptance_rate)}
						</li>
						<li>
							Financial aid & debt:{" "}
						...
						<li>
							Typical earnings: {formatCurrency(university.typical_earnings)}
						</li>
					</ul>
				</article>
			</div>
