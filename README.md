...
- `fastapi_backend/` – FastAPI service that proxies the U.S. Department of
  ...
  persistence in SQL Server, email/SMS utilities, and exposes JSON routes for
  the frontend.
...
- Python 3.11+
- Node.js 18+
- Microsoft SQL Server (with ODBC Driver 18) accessible to the backend
...
cp .env.example .env
# edit .env with SQL Server creds and COLLEGE_SCORECARD_API_KEY
...
- `DATABASE_URL` – use the `mssql+aioodbc:///?odbc_connect=...` format shown in
  `.env.example`.
...
1. Start SQL Server locally or point `DATABASE_URL` to an accessible instance.
...
4. Hit `http://localhost:5173` in a browser; the UI will call the backend, which
   relays live College Scorecard data and persists education plans to SQL Server.
...
- Use `alembic upgrade head` during deployment to keep SQL Server schema in sync.
...
variables, ensure SQL Server accepts connections from the backend host, and
